...
export async function getPlanosAlunos(req, res, next) {
  try {
    ...
    const planosAlunos = await db.collection('planosalunos').find(query).toArray()
...
export async function getPlanoAlunoById(req, res, next) {
  try {
    ...
    const planoAluno = await db.collection('planosalunos').findOne(query)
    if (!planoAluno) {
...
export async function createPlanoAluno(req, res, next) {
  try {
    ...
    const result = await db.collection('planosalunos').insertOne(novoPlanoAluno)
    
    const planoAlunoSalvo = await db.collection('planosalunos').findOne({ _id: result.insertedId })
...
export async function updatePlanoAluno(req, res, next) {
  try {
    ...
    const planoAlunoExistente = await db.collection('planosalunos').findOne(query)
    ...
    const result = await db.collection('planosalunos').updateOne(
      query,
    ...
    const planoAlunoAtualizado = await db.collection('planosalunos').findOne(query)
...
export async function deletePlanoAluno(req, res, next) {
  try {
    ...
    const result = await db.collection('planosalunos').deleteOne(query)
